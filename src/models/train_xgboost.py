"""Phase 4, Step 2: train and evaluate the second baseline model (XGBoost).

Same setup as the Random Forest baseline (src/models/train_random_forest.py)
for direct comparability: loads the same assembled final feature matrices for
the train and validation splits, fits an XGBoost classifier, and evaluates on
validation only. The test split is never loaded here.
...
the top 20 XGBoost feature importances by name.
...
Outputs:
  ...
  src/models/artifacts/xgboost.json
"""
...
METRICS_PATH = ARTIFACTS_DIR / "xgboost.json"
...
def main() -> None:
    print("=" * 78)
    print("PHASE 4 STEP 2: XGBOOST BASELINE")
    print("=" * 78)
    ...
    # Classes are already 50/50 from upstream sampling, so scale_pos_weight is
    # left at default (1). Setting it here would be a no-op on balanced data and
    # only adds noise to the comparison across baseline models.
    print(f"\nFitting XGBClassifier({XGB_PARAMS}) ...")
    ...
    metrics = {
        ...
        "class_weight_note": (
            "scale_pos_weight left at default (1); train split is already 50/50 "
            "phishing/legit from upstream sampling, so adjusting it would be a no-op"
        ),
        "data": {
