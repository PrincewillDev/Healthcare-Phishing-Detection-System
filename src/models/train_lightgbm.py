"""Phase 4, Step 3: train and evaluate the third baseline model (LightGBM).

Same setup as the Random Forest and XGBoost baselines (train_random_forest.py,
train_xgboost.py) for direct comparability: loads the same assembled final
feature matrices for the train and validation splits, fits a LightGBM
classifier, and evaluates on validation only. The test split is never loaded
here.

Evaluation covers precision / recall / F1 / false positive rate / AUC-ROC and
the confusion matrix, reported overall and broken down by source_dataset, plus
the top 20 LightGBM feature importances by name.
...
Outputs:
  src/models/artifacts/lightgbm.pkl
  src/models/artifacts/lightgbm.json
"""
...
METRICS_PATH = ARTIFACTS_DIR / "lightgbm.json"
...
def main() -> None:
    print("=" * 78)
    print("PHASE 4 STEP 3: LIGHTGBM BASELINE")
    print("=" * 78)
    ...
    # Classes are already 50/50 from upstream sampling, so class_weight/
    # scale_pos_weight is left at default. Adjusting it here would be a no-op
    # on balanced data and only adds noise to the comparison across baselines.
    print(f"\nFitting LGBMClassifier({LGBM_PARAMS}) ...")
    ...
    # LightGBM's raw "gain" importances are not normalized to sum to 1, unlike
    # RandomForestClassifier and XGBClassifier. Normalize here so the reported
    # magnitudes are comparable across all three baseline JSON reports.
    ...
    metrics = {
        ...
        "class_weight_note": (
            "left at default; train split is already 50/50 phishing/legit from "
            "upstream sampling, so class_weight/scale_pos_weight adjustment would "
            "be a no-op"
        ),
        "data": {
